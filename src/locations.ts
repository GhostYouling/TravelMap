...
export function inferCountryCode(locationName: string) {
  const prefix = locationName.split("·", 1)[0]?.trim();
  return COUNTRIES.find((country) => country.name === prefix)?.code || null;
}

export function inferCityName(locationName: string) {
