...
- 观看登录使用签名 Cookie，无需额外的用户表或常驻会话服务。
- SQLite 使用 WAL 模式保存旅行与媒体索引；原文件直接保存在磁盘。
- 上传使用磁盘流，不会把整个视频读进 2 GiB 内存。
- 国家列表在浏览器内生成；城市按需查询，服务端只保留最多 200 条短时缓存，不在 2 GiB 内存中加载全球城市库。
...
- 可拖动、自动旋转的地球和旅行地点气泡
- 选择国家、搜索城市，并自动填写 WGS84 经纬度（仍可手动微调）
- 新增、编辑、删除旅程
- 照片/视频多选上传与逐文件进度
- 照片缩略图、图片灯箱、视频播放
- 原文件单独下载、整段旅程 ZIP 下载
- 同一管理口令同时保护影像查看与写操作；未登录无法访问缩略图、原文件或 ZIP
- 响应式界面，适配手机和桌面
...
城市搜索通过后端访问 [Open-Meteo Geocoding API](https://open-meteo.com/en/docs/geocoding-api)，无需 API Key；部署环境需要允许访问 `geocoding-api.open-meteo.com:443`。查询结果会在进程内缓存 30 分钟。
...
编辑 `/opt/jiyu/.env`，至少把 `ADMIN_TOKEN` 换成随机长字符串。它同时用于登录查看影像和授权管理操作。建议生成方式：
...
| `ADMIN_TOKEN` | 空 | 同一口令用于影像观看登录及新增、上传、编辑、删除操作 |
| `SEED_DEMO` | 开发为 `true` | 是否在空库生成演示旅程 |

## 访问控制

设置 `ADMIN_TOKEN` 后，旅行地图与文字信息仍可打开，但以下接口必须先登录：媒体列表、缩略图、照片/视频原文件、单文件下载和旅程 ZIP。登录成功后，后端签发一个 12 小时有效的 `HttpOnly`、`SameSite=Strict` Cookie；媒体响应使用 `no-store`，退出后不会继续通过浏览器缓存展示。管理写操作仍额外校验浏览器标签页中的管理口令。
